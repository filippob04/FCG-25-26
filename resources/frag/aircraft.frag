...
uniform sampler2D aircraftTexture;
...
void main()
{
    ...
    vec4 res = texture(aircraftTexture, texcoord);
    
    // Ambient
    vec3 ambient = res.rgb * light.ambient_val * 2.5;

    // Diffuse
    vec3 light_dir = normalize (light.direct_pos);
    ...
    float spec = pow (max (dot (view_dir, reflect_dir), 0.0), material.shininess * 1.5);
